...
#ifdef DISPLAYMSG
	nxLogPrintf("Transceiver link state :\n");
	if (state & PHY_LINK_RUNNING)
		nxLogPrintf(" Running at ");
	else
		nxLogPrintf(" NOT running at ");
	if (state & PHY_LINK_100MBPS)
		nxLogPrintf("100 Mbps ");
	else
		if (state & PHY_LINK_10MBPS)
			nxLogPrintf("10 Mbps ");
		else
			nxLogPrintf("unknown speed ");
	if (state & PHY_LINK_FULL_DUPLEX)
		nxLogPrintf("in full duplex mode\n");
	else
		if (state & PHY_LINK_HALF_DUPLEX)
			nxLogPrintf("in half duplex mode\n");
		else
			nxLogPrintf("in unknown duplex mode\n");
#endif
...
int Pktdrv_Init(void)
{
	...
	if (!g_s)
	{
		nxLogPrintf("Can't allocate global structure.\n");
		return 0;
	}
	...
	if (!buffers_addr)
	{
		nxLogPrintf("Can't allocate DMA reception buffers\n");
		free(g_s);
		return 0;
	}
	...
	if (len!=6)
		nxLogPrintf("Can't read ethernet address from EEPROM\n");
#ifdef DISPLAYMSG
	else
		nxLogPrintf("EEPROM MacAddress = %02x %02x %02x %02x %02x %02x\n",
				g_s->Ethaddr[0],
				g_s->Ethaddr[1],
				g_s->Ethaddr[2],
				g_s->Ethaddr[3],
	...
	{
		nxLogPrintf("PhyInitialize error\n");
		Pktdrv_Quit();
		return 0;
	}
	...
	if (status==0)
	{
		nxLogPrintf("KeConnectInterrupt error\n");
		Pktdrv_Quit();
		return 0;
	}
#ifdef DISPLAYMSG
	else
		nxLogPrintf("Network interruption initialized successfully\n");
#endif
	return 1;
}
